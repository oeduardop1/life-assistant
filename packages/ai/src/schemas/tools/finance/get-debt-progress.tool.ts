...
export const getDebtProgressParamsSchema = z.object({
  ...
  monthYear: z.string().regex(/^\d{4}-(0[1-9]|1[0-2])$/).optional()
    .describe('Filtrar dividas visiveis neste mes (YYYY-MM). Aplica regras de visibilidade por periodo.'),
});
 ...
 * Supports filtering by month to show only debts visible in that period.
...
export const getDebtProgressTool: ToolDefinition<typeof getDebtProgressParamsSchema> = {
  ...
  description: 'Retorna progresso de pagamento das dividas. Filtra por mes se monthYear fornecido. Inclui parcelas pagas, restantes, percentual de conclusao e status overdue.',
