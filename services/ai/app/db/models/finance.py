...
from sqlalchemy import TIMESTAMP, Boolean, Date, Integer, Numeric, String, Text, func
...
class Income(Base, TimestampMixin):
    ...
    type: Mapped[IncomeType] = mapped_column(String(20))
    frequency: Mapped[IncomeFrequency] = mapped_column(String(20))
    ...
    status: Mapped[IncomeStatus] = mapped_column(String(20), server_default="active")
...
class Bill(Base, TimestampMixin):
    ...
    user_id: Mapped[_uuid.UUID] = mapped_column(UUID(as_uuid=True))
    name: Mapped[str] = mapped_column(String(255))
    category: Mapped[BillCategory] = mapped_column(String(20))
    amount: Mapped[float] = mapped_column(Numeric(precision=12, scale=2, asdecimal=False))
    due_day: Mapped[int] = mapped_column(Integer)
    status: Mapped[BillStatus] = mapped_column(String(20), server_default="pending")
...
class VariableExpense(Base, TimestampMixin):
    ...
    month_year: Mapped[str] = mapped_column(String(7))
    currency: Mapped[str] = mapped_column(String(3), server_default="BRL")
    status: Mapped[ExpenseStatus] = mapped_column(String(20), server_default="active")
...
class Debt(Base, TimestampMixin):
    ...
    status: Mapped[DebtStatus] = mapped_column(String(20), server_default="active")
...
class Investment(Base, TimestampMixin):
    ...
    user_id: Mapped[_uuid.UUID] = mapped_column(UUID(as_uuid=True))
    name: Mapped[str] = mapped_column(String(255))
    type: Mapped[InvestmentType] = mapped_column(String(20))
...
class Budget(Base, TimestampMixin):
    ...
    category: Mapped[ExpenseCategory | None] = mapped_column(String(20), nullable=True)
    amount: Mapped[float] = mapped_column(Numeric(precision=12, scale=2, asdecimal=False))
    currency: Mapped[str] = mapped_column(String(3), server_default="BRL")
